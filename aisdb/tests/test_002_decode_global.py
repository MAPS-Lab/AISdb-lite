...
def test_decode_1day_postgres():
    ...
    testing_data_nm4 = os.path.join(
        os.path.dirname(__file__), "testdata", "test_data_20211101.nm4"
    )
    testing_data_nmea = os.path.join(
        os.path.dirname(__file__), "testdata", "test_data_201201.nmea"
    )
    ...
    with PostgresDBConn(conn_str) as db_conn:
        ...
        # decoding the same files a second time must be idempotent
        decode_msgs(
            filepaths=filepaths,
            dbconn=db_conn,
            source="TESTING",
